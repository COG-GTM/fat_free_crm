...
| `GET /api/v1/contacts` | `page`, `perPage`/`per_page` (clamped to 200), `query` (name/email/phone; two words match first/last in either order), `sortBy`/`sort_by` (`first_name ASC`, `last_name ASC`, `created_at DESC`, `updated_at DESC`) |
